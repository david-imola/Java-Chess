...
public class Pawn extends Piece {
	...
	public boolean isImpeded(Board board, Coordinate coord) { // implemented
																...
																// returns true
																// if Pawn is
		return (!board.isEmpty(new Coordinate(coord.getNum() + forwardInc, coord.getLetter()))); // impeded
																									// on
																									// it's
																									// path
																									// to
	...
	public boolean testMove(Board board, Coordinate coord) { // I'll do this, as
		...
		if (super.isSameColor(p)) // checks whether the piece is of the same
			...
			if (super.getLetter() == letter) // If piece is on same letter rank
			...
			else if ((Math.abs(super.getLetter() - letter) == 1) && (super.getNum() == num + forwardInc)) {
				Player oppPlayer;
				...
				String last = oppPlayer.getLastMove().substring(0, 2);
				String not = Coordinate.notatedPos(num + forwardInc, letter);
				if(last.equals(not)) {
					Coordinate pawnRemove = Coordinate.decode(not);
					if(board.getLocAt(pawnRemove).getPiece() instanceof Pawn ) {
						oppPlayer.removePiece(board.getLocAt(pawnRemove).getPiece());
						board.getLocAt(pawnRemove).setPiece(null);
						board.removePieceGUI(pawnRemove);
						return true;
					}
				}
				return false;
			...
			return ((Math.abs(super.getLetter() - letter) == 1) && (super.getNum() == num + forwardInc)); // take
																											// move
